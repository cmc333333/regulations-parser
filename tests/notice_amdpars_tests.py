...
from regparser.notice import amdpars


class NoticeAmdparsTests(TestCase):
    ...
    def test_clear_between(self):
        ...
        """.strip()
        result = amdpars.clear_between(etree.fromstring(xml), '[', ']')
        cleaned = u"""
        <ROOT>Some content
            <CHILD>Split
            </CHILD>
    ...
    def test_remove_char(self):
        ...
        result = amdpars.remove_char(amdpars.remove_char(
    ...
    def test_make_amendments(self):
        ...
        amends = amdpars.make_amendments(tokenized)
        self.assertEqual(amends,
                         [amdpars.Amendment(tokens.Verb.PUT, '222'),
                          amdpars.Amendment(tokens.Verb.PUT, '333'),
                          amdpars.Amendment(tokens.Verb.PUT, '444'),
                          amdpars.Amendment(tokens.Verb.DELETE, '555'),
                          amdpars.Amendment(tokens.Verb.MOVE, '666', '777')])
    ...
    def test_compress_context_simple(self):
        ...
        converted, final_ctx = amdpars.compress_context(tokenized, [])
    ...
    def test_compress_context_initial_context(self):
        ...
        converted, _ = amdpars.compress_context(tokenized, ['111', None, '12'])
        self.assertEqual(converted,
    ...
    def test_compress_context_interpretations(self):
        ...
        converted, _ = amdpars.compress_context(tokenized, [])
        self.assertEqual(converted, [
    ...
    def test_compress_context_in_tokenlists(self):
        ...
        converted = amdpars.compress_context_in_tokenlists(tokenized)
        self.assertEqual(converted, [
            tokens.Context(['123', 'Interpretations']),
            tokens.Paragraph(['123', None, '23', 'a']),
            tokens.Verb(tokens.Verb.PUT, True),
    ...
    def test_resolve_confused_context(self):
        ...
        converted = amdpars.resolve_confused_context(
    ...
    def test_compress(self):
        self.assertEqual([1, 2, 3], amdpars.compress([1, 2, 3], []))
        self.assertEqual([1, 6, 3],
                         amdpars.compress([1, 2, 3, 4, 5], [None, 6, None]))
        self.assertEqual([2, 2, 5, 6],
                         amdpars.compress([1, 2], [2, None, 5, 6]))
    ...
    def test_separate_tokenlist(self):
        ...
        converted = amdpars.separate_tokenlist(tokenized)
        self.assertEqual(converted, [
            tokens.Context(['1']),
            tokens.Verb(tokens.Verb.MOVE, active=True),
            tokens.Context([None, '2']),
    ...
    def test_context_to_paragraph(self):
        ...
        converted = amdpars.context_to_paragraph(tokenized)
        self.assertEqual(converted, [
            tokens.Context(['1']),
            tokens.Verb(tokens.Verb.PUT, active=True),
            tokens.Paragraph(['2']),
    ...
    def test_switch_passive(self):
        ...
        converted = amdpars.switch_passive(tokenized)
        self.assertEqual(tokenized, converted)
    ...
    def test_find_section(self):
        ...
        section = amdpars.find_section(amdpar_xml)
    ...
    def test_find_subpart(self):
        ...
        notice_xml = etree.fromstring(xml)
        amdpar_xml = notice_xml.xpath('//AMDPAR')[0]
        subpart = amdpars.find_subpart(amdpar_xml)
    ...
    def test_is_designate_token(self):
        ...
        self.assertTrue(amdpars.is_designate_token(token))

        token = tokens.Verb(tokens.Verb.MOVE, True)
        self.assertFalse(amdpars.is_designate_token(token))
    ...
    def test_contains_one_designate_token(self):
        ...
        self.assertTrue(amdpars.contains_one_designate_token(tokenized))

        designate_token_2 = tokens.Verb(tokens.Verb.DESIGNATE, True)
        tokenized.append(designate_token_2)
        self.assertFalse(amdpars.contains_one_designate_token(tokenized))
    ...
    def test_contains_one_tokenlist(self):
        ...
        self.assertTrue(amdpars.contains_one_tokenlist(tokenized))

        tokenized = [tokens.TokenList(token_list),
                     designate_token_2, tokens.TokenList(token_list)]
        self.assertFalse(amdpars.contains_one_tokenlist(tokenized))
    ...
    def test_contains_one_context(self):
        ...
        self.assertTrue(amdpars.contains_one_context(tokenized))

        designate_token = tokens.Verb(tokens.Verb.DESIGNATE, True)
        tokenized.append(designate_token)
        tokenized.append(tokens.Context(['200', '2']))

        self.assertFalse(amdpars.contains_one_context(tokenized))
    ...
    def test_deal_with_subpart_adds(self):
        ...
        toks, subpart_added = amdpars.deal_with_subpart_adds(tokenized)
        ...
        self.assertTrue(amdpars.contains_one_tokenlist(toks))
        self.assertTrue(amdpars.contains_one_designate_token(toks))
    ...
    def test_get_destination_normal(self):
        ...
        self.assertEqual(amdpars.get_destination(tokenized, '205'),
                         '205-Subpart-A')
    ...
    def test_handle_subpart_designate(self):
        ...
        amendment = amdpars.handle_subpart_amendment(tokenized)
    ...
    def test_make_amendments_subpart(self):
        ...
        subpart_token = tokens.Paragraph([None, 'Subpart', 'J'])
        tokenized = [token_list, subpart_token]
        amends = amdpars.make_amendments(tokenized, subpart=True)
    ...
    def test_new_subpart_added(self):
        amended_label = amdpars.Amendment('POST', '200-Subpart:B')
        self.assertTrue(amdpars.new_subpart_added(amended_label))

        amended_label = amdpars.Amendment('PUT', '200-Subpart:B')
        self.assertFalse(amdpars.new_subpart_added(amended_label))

        amended_label = amdpars.Amendment('POST', '200-Subpart:B-a-3')
        self.assertFalse(amdpars.new_subpart_added(amended_label))
    ...
    def test_switch_context(self):
        ...
        self.assertEqual(amdpars.switch_context(tokenized, initial_context),
                         [])
    ...
    def test_fix_section_node(self):
        ...
        section = amdpars.fix_section_node(paragraphs, par)
    ...
    def test_find_section_paragraphs(self):
        ...
        section = amdpars.find_section(amdpar)
        self.assertNotEqual(None, section)
    ...
    def test_find_lost_section(self):
        ...
        amdpar = etree.fromstring(amdpar_xml).xpath('//AMDPAR')[0]
        section = amdpars.find_lost_section(amdpar)
        self.assertNotEqual(None, section)
    ...
    def test_remove_false_deletes(self):
        ...
        new_tokenized = amdpars.remove_false_deletes(tokenized, text)
    ...
    def test_multiple_moves(self):
        ...
        tokenized = amdpars.multiple_moves(tokenized)
        self.assertEqual(
        ...
        tokenized = [
            ...
            tokens.Verb(tokens.Verb.MOVE, active=False),
            tokens.TokenList([tokens.Paragraph(['444', '3'])])]
        self.assertEqual(tokenized, amdpars.multiple_moves(tokenized))
    ...
    def test_parse_amdpar_interp_phrase(self):
        ...
        amends, _ = amdpars.parse_amdpar(xml, ['1111'])
    ...
    def test_parse_amdpar_interp_heading(self):
        ...
        xml = etree.fromstring(u'<AMDPAR>%s</AMDPAR>' % text)
        amends, _ = amdpars.parse_amdpar(xml, ['1111', 'Interpretations'])
        self.assertEqual(1, len(amends))
    ...
    def test_parse_amdpar_add_field(self):
        ...
        xml = etree.fromstring('<AMDPAR>%s</AMDPAR>' % text)
        amends, _ = amdpars.parse_amdpar(xml, ['1111', None, '12'])
        self.assertEqual(1, len(amends))
        amd = amends[0]
...
class AmendmentTests(TestCase):
    def test_fix_label(self):
        amd = amdpars.Amendment('action', '1005-Interpretations')
        self.assertEqual(amd.label, ['1005', 'Interp'])

        amd = amdpars.Amendment('action', '1005-Interpretations-31-(b)(1)-3')
        self.assertEqual(amd.label, ['1005', '31', 'b', '1', 'Interp', '3'])

        amd = amdpars.Amendment('action',
                                '1005-Interpretations-31-(b)(1)-3[title]')
        self.assertEqual(amd.label, ['1005', '31', 'b', '1', 'Interp', '3'])

        amd = amdpars.Amendment('action', '1005-Interpretations-31-(c)-2-xi')
        self.assertEqual(amd.label, ['1005', '31', 'c', 'Interp', '2', 'xi'])

        amd = amdpars.Amendment('action', '1005-Interpretations-31-()-2-xi')
        self.assertEqual(amd.label, ['1005', '31', 'Interp', '2', 'xi'])

        amd = amdpars.Amendment('action', '1005-Interpretations-Appendix:A-2')
        self.assertEqual(amd.label, ['1005', 'A', '2', 'Interp'])

        amd = amdpars.Amendment('action', '1005-Appendix:A-2')
        self.assertEqual(amd.label, ['1005', 'A', '2'])

    def test_amendment_heading(self):
        amendment = amdpars.Amendment('PUT', '100-2-a[heading]')
...
class DesignateAmendmentTests(TestCase):
    def test_fix_interp_format(self):
        amd = amdpars.DesignateAmendment(
